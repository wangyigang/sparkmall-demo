package com.wangyg.sparkmall.comm
...
package object xbean {
  ...
  /**
    *   taksid --sql任务id --uuid随机生成 category_id  品种id, clickcount--点击次数  orderCount--订单次数
    *   payCount --支付次数
    * @param taskid
    * @param category_id
    * @param clickCount
  ...
  case class CategoryCountInfo(taskid: String, category_id: String,
                               clickCount: Long, orderCount: Long, payCount: Long)

  case class KafkaMessage(timestamp:String, area:String,
                          city:String, userid:String, adid:String)
