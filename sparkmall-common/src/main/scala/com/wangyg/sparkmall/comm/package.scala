package com.wangyg.sparkmall.common
...
package object xutil {
  ...
  object RandomNum {
    ...
    def multi(fromNum: Int, toNum: Int, amount: Int, delimiter: String, canRepeat: Boolean) = {
      ...
      //
      //        var a = fromNum + (new Random).nextInt((toNum - 1) + 1)
      //        if (a <= fromNum || a >= toNum) {
      //          s.append(a) + delimiter
      //        }
      //      }
      //
      //      s.toString()
    ...
    def getKafkaStream(topic: String,ssc:StreamingContext): InputDStream[ConsumerRecord[String,String]]={
      val dStream = KafkaUtils.createDirectStream[String,String](ssc,
        LocationStrategies.PreferConsistent,ConsumerStrategies.
          Subscribe[String,String](Array(topic),kafkaParam))
      dStream
    }
  }
